...
def normalize_settings(settings):
    ...
    # this duplicates the logic in export_config_dict() in firewall.core.io.io_object.IO_Object
...
def config_to_dict(module, detailed=None, online=None):
    ...
    custom_permanent_with_defaults = fetch_settings_from_xml_files(
        module, setting_list, defaults=False
    )
    config["custom_permanent_with_defaults"] = custom_permanent_with_defaults
    # the custom permanent settings include the defaults, so we need to diff them to get the custom permanent settings
    dict_diff_normalizers = DEFAULT_NORMALIZERS
    custom_permanent = recursive_dict_diff(
        custom_permanent_with_defaults, defaults, dict_diff_normalizers
    )
    if custom_permanent:
    ...
    if online:
        ...
        config["current"] = current_settings
        config["custom_runtime_with_defaults"] = current_settings
        ...
        custom_runtime = recursive_dict_diff(
            current_settings, custom_permanent_with_defaults, dict_diff_normalizers
        )
        if custom_runtime:
...
def fetch_settings_from_xml_files(module, setting_list, defaults=False):
    ...
    for setting_name in setting_list:
        ...
        setting_dir = os.path.join(root_dir, setting_name)
        ...
        try:
            xml_files = [f for f in os.listdir(setting_dir) if f.endswith(".xml")]
        except OSError:
            continue
        ...
        for xml_file in xml_files:
            ...
            file_path = os.path.join(setting_dir, xml_file)
            ...
            try:
                ...
                # Extract settings from the object
                element_settings = {}

                if setting_name == "zones":
                    try:
                        element_settings = obj.export_config_dict()
                    except AttributeError:
                        element_settings["version"] = obj.version
                        element_settings["short"] = obj.short
                        element_settings["description"] = obj.description
                        element_settings["target"] = obj.target
                        element_settings["services"] = obj.services
                        element_settings["ports"] = obj.ports
                        element_settings["icmp_blocks"] = obj.icmp_blocks
                        element_settings["masquerade"] = obj.masquerade
                        element_settings["forward_ports"] = obj.forward_ports
                        element_settings["interfaces"] = obj.interfaces
                        element_settings["sources"] = obj.sources
                        element_settings["rules_str"] = obj.rules_str
                        element_settings["protocols"] = obj.protocols
                        element_settings["source_ports"] = obj.source_ports
                        element_settings["icmp_block_inversion"] = (
                            obj.icmp_block_inversion
                        )
                    if (
                        element_settings["target"]
                        == firewall.core.base.DEFAULT_ZONE_TARGET
                    ):
                        element_settings["target"] = (
                            "default"  # to correspond with online getTarget()
                        )
                    ...
                    try:
                        ...
                        element_settings["protocols"] = obj.protocols
                        element_settings["source_ports"] = obj.source_ports
                        element_settings["modules"] = obj.modules

                elif setting_name == "icmptypes":
                    element_settings["version"] = obj.version
                    element_settings["short"] = obj.short
                    element_settings["description"] = obj.description
                    element_settings["destination"] = obj.destination

                elif setting_name == "helpers":
                    element_settings["version"] = obj.version
                    element_settings["short"] = obj.short
                    element_settings["description"] = obj.description
                    element_settings["family"] = obj.family
                    element_settings["module"] = obj.module
                    element_settings["ports"] = obj.ports
                    ...
                    element_settings["short"] = obj.short
                    element_settings["description"] = obj.description
                    element_settings["type"] = obj.type
                    element_settings["options"] = obj.options
                    element_settings["entries"] = obj.entries
                    ...
                    try:
                        ...
                        element_settings["source_ports"] = obj.source_ports
                        element_settings["rules_str"] = obj.rules_str
                        element_settings["ingress_zones"] = obj.ingress_zones
                        element_settings["egress_zones"] = obj.egress_zones

                settings[item_name] = normalize_settings(element_settings)
