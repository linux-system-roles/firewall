...
class TestRecursiveShowDiffs(unittest.TestCase):
    ...
    def test_dict1_none(self):
        ...
        self.assertEqual(result, {"removed": None, "added": dict2})

    def test_dict2_none(self):
        """Test when dict2 is None"""
        dict1 = {"key": "value"}
        result = recursive_show_diffs(dict1, None, None)
        self.assertEqual(result, {"removed": dict1, "added": None})
    ...
    def test_simple_diff(self):
        """Test simple value difference shows removed and added"""
        dict1 = {"key": "value1"}
        dict2 = {"key": "value2"}
        result = recursive_show_diffs(dict1, dict2, None)
        self.assertEqual(result["removed"], {"key": "value1"})
        self.assertEqual(result["added"], {"key": "value2"})
    ...
    def test_key_only_in_dict1(self):
        ...
        dict2 = {"key1": "value1"}
        result = recursive_show_diffs(dict1, dict2, None)
        self.assertEqual(result["removed"], {"key2": "value2"})
    ...
    def test_key_only_in_dict2(self):
        ...
        self.assertEqual(result["added"], {"key2": "value2"})
    ...
    def test_nested_dict_diff(self):
        ...
        self.assertEqual(result["removed"], {})
        self.assertEqual(
            result["added"]["zones"]["internal"]["ports"],
            (("8443", "tcp"),),
        )
    ...
    def test_deeply_nested_diff(self):
        ...
        result = recursive_show_diffs(dict1, dict2, None)
        self.assertEqual(
            result["removed"], {"level1": {"level2": {"level3": {"value": "old"}}}}
        )
        self.assertEqual(
            result["added"], {"level1": {"level2": {"level3": {"value": "new"}}}}
        )

    def test_list_diff(self):
        """Test list differences: only elements unique to each side"""
        dict1 = {"items": ["a", "b"]}
        dict2 = {"items": ["a", "b", "c"]}
        result = recursive_show_diffs(dict1, dict2, None)
        self.assertEqual(result["removed"], {})
        self.assertEqual(result["added"]["items"], ["c"])
    ...
    def test_list_identical_different_order(self):
        """Test identical lists with different order"""
        dict1 = {"items": ["a", "b", "c"]}
        dict2 = {"items": ["c", "b", "a"]}
        result = recursive_show_diffs(dict1, dict2, None)
        self.assertIsNone(result)
    ...
    def test_multiple_differences(self):
        ...
        self.assertEqual(result["added"]["a"], 10)
        self.assertEqual(result["removed"]["c"], 3)
        self.assertEqual(result["added"]["c"], 30)
        self.assertNotIn("b", result["removed"])
        self.assertNotIn("b", result["added"])
    ...
    def test_complex_nested_diff(self):
        ...
        result = recursive_show_diffs(dict1, dict2, None)
        self.assertIsNotNone(result)
        self.assertIn("zones", result["removed"])
        self.assertIn("zones", result["added"])
        self.assertIn("public", result["removed"]["zones"])
        self.assertIn("public", result["added"]["zones"])
        # services: only https added; target changed as scalars
        self.assertNotIn("services", result["removed"]["zones"]["public"])
        self.assertIn("services", result["added"]["zones"]["public"])
        self.assertEqual(result["added"]["zones"]["public"]["services"], ["https"])
        self.assertIn("target", result["removed"]["zones"]["public"])
        self.assertIn("target", result["added"]["zones"]["public"])
        self.assertEqual(result["removed"]["zones"]["public"]["target"], "default")
        self.assertEqual(result["added"]["zones"]["public"]["target"], "ACCEPT")
        self.assertNotIn("ports", result["removed"]["zones"]["public"])
        self.assertNotIn("ports", result["added"]["zones"]["public"])
        # internal zone should not be in diff
        self.assertNotIn("internal", result["removed"]["zones"])
        self.assertNotIn("internal", result["added"]["zones"])
        # customzone should be in diff
        self.assertNotIn("customzone", result["removed"]["zones"])
        self.assertEqual(result["added"]["zones"]["customzone"], customzone)
    ...
    def test_non_dict_values_with_diff(self):
        ...
        self.assertEqual(result, {"removed": "old", "added": "new"})
    ...
    def test_list_values_directly(self):
        ...
        self.assertEqual(result, {"removed": [], "added": [4]})
