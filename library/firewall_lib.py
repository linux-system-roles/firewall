...
class InMemoryBackend:
    ...
    def _new_zone(self):
        if HAS_FIREWALLD:
            new_zone = Zone().export_config_dict()
            if (
                "target" in new_zone
                and new_zone["target"] == firewall.core.base.DEFAULT_ZONE_TARGET
            ):
                new_zone["target"] = "default"
            return new_zone
        else:
            return {}
    ...
    def _new_service(self):
        if HAS_FIREWALLD:
            new_service = Service().export_config_dict()
            return new_service
        else:
            return {}
    ...
    def _new_ipset(self, ipset_type):
        if HAS_FIREWALLD:
            new_ipset = IPSet().export_config_dict()
...
def check_for_diffs(
    ...
    permanent_diff = recursive_show_diffs(
        permanent_config, custom_permanent_with_defaults, None
    )
    if online:
        runtime_diff = recursive_show_diffs(
            runtime_config,
        ...
        runtime_diff = {}
    changed = any(
        (permanent_diff, runtime_diff, original_config["default_zone"] != default_zone)
    )
    diff = {}
    if permanent_diff:
        diff["permanent_diff"] = permanent_diff
    if online and runtime_diff:
        diff["runtime_diff"] = runtime_diff
    if original_config["default_zone"] != default_zone:
        diff["before_default_zone"] = original_config["default_zone"]
        diff["after_default_zone"] = default_zone
    ...
    if replaced and (changed or custom_permanent_with_defaults):
        ...
        if changed or not default_diffs:
            for root, dirs, files in os.walk(FIREWALLD_DIR):
                for filename in files:
                    if filename.endswith(".xml"):
                        xml_file = os.path.join(root, filename)
                        if os.path.exists(xml_file):
                            custom_file_list.append(xml_file)
        # check if there are any custom files to be removed
        if custom_file_list:
            diff["files_to_be_removed"] = custom_file_list
        if not changed and custom_file_list:
            module.warn(
                "Found custom files in %s which were the same as the defaults - see files_to_be_removed in the diffs"
                % FIREWALLD_DIR
            )
    return (changed, diff, custom_file_list)
...
def process_replaced_config(
    ...
    if not module.check_mode and (changed or custom_file_list):
        ...
        for xml_file in custom_file_list:
            try:
                os.remove(xml_file)
...
def main():
    ...
    if config_list:
        ...
        changed, diff, custom_file_list = check_for_diffs(
            module,
            filtered_config_list,
            has_replaced,
            online_param=online,
        ...
        if has_replaced and (changed or custom_file_list):
            # This handles removing the files that need to be removed and reloading firewalld if necessary
            process_replaced_config(module, changed, custom_file_list, online)

        if not changed:
            # The desired configuration is exactly the same as the current configuration
            module.exit_json(changed=False, __firewall_changed=False, diff=diff)
        ...
        if not processing_change:
            module.warn(
                "check_for_diffs reported changes but processing reported no changes"
            )
        module.exit_json(changed=changed, __firewall_changed=changed, diff=diff)
